...
export class PollsRepository {
  ...
  async removeParticipant(pollID: string, userID: string): Promise<Poll> {
    this.logger.log(`removing userID ${userID} form poll ${pollID}`);
  ...
  async startPoll(pollID: string): Promise<Poll> {
    ...
    try {
      ...
      this.logger.error(`Failed to set hasStarted to poll ${pollID}`, e);
  ...
  async addParticipantRankings({
    ...
    this.logger.log(
      `Attempting to add rankings for userID/username ${userID} to pollID ${pollID}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to add a rankings for user ${userID} to pollID ${pollID}`,
      );
